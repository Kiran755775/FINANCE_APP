        ...
        <div className="bg-[#3273db] sm:h-[80vh]   sm:top-24 sm:w-[18vw] sm:rounded-[30px]  h-[100vh] w-[100vw]  relative overflow-auto ">
          <div>
            <div className=" sticky top-0 left-0 right-0 z-10 bg-[#3273db] h-[21vh]">
              <div className="flex justify-between pl-4 py-2">
              ...
              <div className="flex justify-between px-3 items-center ">
                <div className="flex items-center">
                  ...
                  />
                  <div className="pl-2">
                    <p className="text-white sm:text-[8px] text-[12px]">
                      Good Morning,
                    </p>
                    <h1 className="text-white sm:text-[5px] text-[17px] sm:text-base font-semibold ">
                      Asal Design
              ...
              <ul className="flex justify-between pt-4 m-2  mx-3">
                {featuresList.map((eachItem) => (
                  <li
                    className={`text-white sm:text-[13px] text-[16px] font-semibold cursor-pointer rounded-2xl p-1    ${
                      eachItem.id === selectId ? "bg-white" : ""
                    }  ${eachItem.id === selectId ? "text-yellow-800" : ""} `}
                    key={eachItem.id}
                    onClick={() => setSelectId(eachItem.id)}
                  >
                    {eachItem.name}
                  </li>
                ))}
            ...
            {featuresList.map((eachItem) =>
              eachItem.id === selectId ? (
                <div key={eachItem.id}>{eachItem.description}</div>
              ) : null
            )}
          </div>
        </div>
